...
- [`cards/memory/kicad/v2.0`](cards/memory/kicad/v2.0/) — see its README (the built memory v1.3 + the CF interface on P8/P9; schematic proven; whole card re-laid (Ken 2026-09-24); **the v2.0 board `memory-v2.0.kicad_pcb` = standoff option E (Ken's pick 2026-09-25): the HX-2118P CF adapter on two 15 mm M3 standoffs (H1/H2, 3.2 mm NPTH) over the CF chips, J2 parallel to X1, a short ribbon; routed 0 unrouted / 69 through vias, DRC 0 copper violations / 0 unconnected, netlist proof MATCH, silkscreen tidied, fab files (gerbers + NPTH/PTH drill zip, renders, placement PDF, BOM with hardware, JLCPCB note, 1:1 print); NOT ORDERED**; records: the standoff options A-E, the top-edge board in `options-top-edge-J2/`, the keep-copper options; C20-C23 removed: 70 parts)
