# blank-card-v3.2 — KiCad conversion

Generated 2026-09-23 by `tools/eagle_to_kicad_all.py` from the Eagle design `Blank V3.2` in `hardware/bus/blank-card/eagle/v3.2/`. **The Eagle files are the record of what was designed and built; this KiCad project is derived from them and is regenerated from scratch on every run of the tool** (hand edits here will be lost - once a card is edited in KiCad, remove it from the tool's list). This revision was never fabricated (or its build status is unknown, see `hardware/FABRICATED.md`).

## Files

| File | What it is |
|---|---|
| `blank-card-v3.2.kicad_pro` | project; OSH Park rules (6 mil track, 5 mil clearance, 10 mil drill, 4 mil annular) |
| `blank-card-v3.2.kicad_sch` | root sheet; 1 sub-sheet(s) `blank-card-v3.2-sheetN.kicad_sch` mirror the Eagle sheets |
| `blank-card-v3.2-eagle.kicad_sym` | project symbol library generated from the Eagle libraries used (units = Eagle gates) |
| `blank-card-v3.2.kicad_pcb` | the board: 2 copper layers, 3 footprints, 21 track segments, 0 vias |
| `blank-card-v3.2-eagle.pretty/` | project footprint library extracted from the imported board |
...
```
  same connectivity, different net name: [('Net-(PWR0-PadA)', 'N$11')]
```
