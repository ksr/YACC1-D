...
Generated 2026-09-20 by `tools/eagle_to_kicad_all.py` from the Eagle design `Jumper Board V3.0` in `hardware/bus/bus-jumper-vertical/eagle/v3.0/`. **The Eagle files are the record of what was designed and built; this KiCad project is derived from them and is regenerated from scratch on every run of the tool** (hand edits here will be lost - once a card is edited in KiCad, remove it from the tool's list). This revision was FABRICATED (see `hardware/FABRICATED.md`).
