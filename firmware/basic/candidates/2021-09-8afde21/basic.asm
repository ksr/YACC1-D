...
charout:    EQU 0ffc4h ;
uartout:    EQU 0ffc8h ; these are the same entry point, remove one?
showaddr:   EQU 0ffcch
toupper:    EQU 0ffd0h
showr7:     EQU 0ffd4h
showbyte:   equ 0ffd8h
...
TOKENIZER_CONTINUE: EQU 47


;
; BASIC interpreter user variables area
; currently 26 A-Z variables
; for now BASIC_VARS must be 0xXX00 (256 byte) aligned
;
...
;
; Basic Interpreter, these entry locations are referenced from monitor.asm
;
...
;
  ORG 0e000h
    ...
    DB "BASIC Code"
;
; list, execute BASIC list command on token buffer
;
...
;
      ORG 0e010h
;
; run, execute BASIC run command om token buffer
;
;      jsr basic_run
;      Ret
...
; R0 - Program Counter
; R1 - Stack Pointer
; R2 - Direct addressing mode, used by XXX instruction(s) , edit in info hack
...
;
; debug information
; Maybe set finished flag and try to get back to command line
;
    JSR showbytea
    ldai '#'
    JSR charout
...
noemu:
   JSR test_input          ; check for keyboard press
    LDA bas_run_ended
    LDTI 1
...
parse_keywords:
      ...
      DB "continue",0,TOKENIZER_CONTINUE
...
parse_line_run:
    ldti TOKENIZER_RUN
    brneq parse_line_CONT
    jsr basic_run
    ldai 0
    RET

parse_line_cont:
    ldti TOKENIZER_CONTINUE
