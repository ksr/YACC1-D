...
### The next card on the ports: CompactFlash on P8/P9
...
- **Two ports, same select-then-data pattern as this card.** P8 = write-only register-select latch (bits 0-2 = the ATA
  ...
  = the CS1 block, optional); P9 = the data port — a read or write of P9 strobes the CF's -IOR/-IOW on the selected
  register.
- **Parts as planned:** a 74245 data buffer, a 74LS174/273 select latch, port decode (IO-ADDR3 high, IO-ADDR0..2 =
  0/1 — so it lives in the *other* half from this card, which stays at P0-P7), strobe gating from -IO-RD/-IO-WR, the
  CF status pull-ups, an activity LED; 8-bit True IDE mode (SET FEATURES $EF with feature $01 at init); otherwise the
  P8X CF card's circuit. First KiCad-native card of the machine.
...
- **The emulators model it** (`software/cfmodel.h`: `cf_io_write(8, v)` sets the select, port 9 reads/writes the
  selected register; no image attached = $FF like a floating bus), so the card can be bench-tested with the bus tester
  (`OUTI P8` / `INP P9` by hand, as the 2020 IO scripts did for the UART) and the driver run on the emulator first.
- Port map afterwards (OS-PLAN decision 3): P0/P1 this card, P2-P7 reserved to it, P8/P9 CF, PA/PB the next video
