# CompactFlash interface (CF card v1.0 circuit) — theory of operation
...
Written 2026-09-23 from the YACC1-D tree, while the card was being designed (not yet built); section 0 updated
2026-09-24, when the interface was planned onto the memory card.
...
## 0. Where the circuit goes (2026-09-24)
...
it: a CF card can draw about 100 mA in bursts. **To verify:** the power connector and pinout of the adapter chosen
(section 0; the TAODAN CF-IDE40 can take +5 V on IDE pin 20 through JP1 instead).
...
The interface needs the I/O card strapped to ports 0..7 (its `IO-ADDR-HL` header), which is how it has always been
used; strapped to P8-P15 the I/O card would answer on P8/P9 too. **To verify** on the bench before plugging the CF
card in (`docs/cards/io.md`). On the memory card the designators will be that design's own.

## 7. Bring-up and test

The ROM in the machine (`ROM 2026-09-23`, burned 2026-09-23) already drives P8/P9; no new burn is needed.
...
2. With the bus tester (port 8 on IO-ADDR0..3, a byte on the data bus, a `-IO-WR` pulse) or a two-instruction program
   entered with the monitor's `E` command (`OUTI P8,n` / `RET`, run with `G`), write to P8 and watch DA0..2 on J1 pins
