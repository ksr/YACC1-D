...
- Bit 3 of the select byte holds the CF in reset while 1 (the hardware's addition; the ROM never sets it and the model
  ignores it).
- The hardware is not built. The circuit is the CF card v1.0's ([`docs/cards/cf.md`](../cards/cf.md)): a 74LS138
  decoding P8/P9 (enabled by IO-ADDR3 high), 74LS32 strobe gating from -IO-RD/-IO-WR, a 74LS175 select latch, a
  74LS08 (buffer enable, CF reset, ACT LED), a 74LS245 data buffer, status pull-ups and a 40-pin IDE header for a
  CF-to-IDE adapter. Since 2026-09-24 it is planned onto the memory card, still I/O-mapped (not designed yet).
  Bench-test with the bus tester before the CPU touches it.
