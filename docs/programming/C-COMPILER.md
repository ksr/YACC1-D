...
| `sys(n, a, b, c)` → int | args to `SYSARG0..2` ($0F06/$0F08/$0F0A), `LDR R7,SYSTAB+2n` / `JSRUR R7`, `LDR R3,SYSRES` | a Y1/OS syscall (2026-09-23): n = 0..31, a constant or computed (a constant 22..31 reads `SYSTAB2` = $4FC0 + 2n instead, 2026-09-25; a computed n indexes `SYSTAB`, so 0..21); `a`, `b`, `c` optional; the result is the full 16-bit word the handler wrote to `SYSRES` ($0F0C). A later argument that calls anything is evaluated with the earlier ones parked on the stack, so `sys(S_ADD, 100, sys(S_ADD, 20, 3))` works (`tests/compiler/syscall.c`) |
