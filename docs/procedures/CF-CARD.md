...
adapter on the CF card v1.0 (`docs/cards/cf.md`), and the monitor's `O` command boots it.
...
5. Put the card in the adapter, the adapter on the CF card's J1, power it through J2, and in the monitor type `O`.
