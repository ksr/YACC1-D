...
| Sequencer-memory ATmega328P | `embedded/sequencer-card/sequencer3` (Sequencer3.ino 2021-08 + IO.ino 2024-07) | flash read out 2026-09-21 (`embedded/sequencer-card/readback/`): 13,290 bytes, its printable strings are exactly the set a build of the tree's Sequencer3 produces (not Sequencer/Sequencer2); byte-identity unprovable because it was built with an older avr-gcc (tree build today: 11,186 bytes). Not re-flashed |
