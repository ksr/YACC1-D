...
| I/O | V1.1 (2020-11) | `hardware/cards/io/eagle/v1.1` | UART behind P0/P1 at 38400, switch/LED port, TIL311s. **V2.0 is being designed** (2026-09-23, `hardware/cards/io/kicad/v2.0`): the same card plus the CompactFlash interface on P4/P5 (`docs/cards/io.md`, `docs/cards/cf.md`); no CF hardware exists yet |
...
|---|---|---|
| Memory card 28C64 EEPROM | **the 2026-09-23 build, burned by Ken 2026-09-23** (Visual Minipro, `firmware/rom/shipped/rom.bin` as it was then, MD5 d2d7b027e7c6951d7dd93412a8fd9cd8): BASIC unchanged; the monitor with G = JSRUR R7, the CompactFlash driver and `O`, the `:` Intel-hex loader, vectors CFINIT/CFREAD/CFWRITE/CONST/UARTINNE at $FFEC..$FFFC; banner `YACC 2020: HELLO WORLD  ROM 2026-09-23` | **first boot on the machine 2026-09-23**: banner, register dump and the full help menu (with `O` and `:`) over the UART at 38400 (`screen /dev/cu.usbserial-AB0MVHSQ 38400`); Ken then used the monitor commands and ROM BASIC interactively, all working (Ken 2026-09-23). The ROM tool chain (assembler -> image -> Visual Minipro -> 28C64 -> boot) is proven. **tests/bench on the machine, 2026-09-23 evening: 14 of 14 PASS** (`tests/bench/logs/bench-2026-09-23-1855.log`): the `:` loader, BRUR, the full ISA sweep and eleven compiled C programs, byte-identical to the microcode emulator, after the SHIFT-OUT carry fix (`docs/cards/alu.md` 3.3). The first run (`bench-2026-09-23-1809.log`) found that fault: hello, brur and isa passed, arith hung. The 2021 chip image is kept as `firmware/rom/eprom-captured-2026-09-18.*`. **The tree has moved on, not yet burned**: `firmware/rom/shipped/rom.bin` is now `ROM 2026-09-23B` (MD5 a9fefd4ae21eb46eb21cff614376617f, 2026-09-23 evening), identical in use except that the CF driver and `O` talk to P4/P5 (the I/O card v2.0) instead of P8/P9. The chip's CF path therefore matches no hardware; burn 2026-09-23B before the v2.0 CF interface is tested. Until then `tests/memory/rom_verify.py` reports the monitor half as differing |
...
- `os/` (2026-09-22) — Y1/OS v0: `make -C os run` boots it on the microcode emulator from `os/disk.img` (P8XFS v2, `tools/p8xfs.py`); both emulators take `-c disk.img` for the CF interface on ports P4/P5 (P8/P9 until 2026-09-23). The hardware is not built yet: it will be the I/O card v2.0.
