...
| Memory | v1.3 (boards ordered 2025-06-27, JLCPCB 2000765A) | `hardware/cards/memory/eagle/v1.3` (+ KiCad `kicad/v1.3`) | see jumpers below. The built design is Ken's Fusion export of 2026-09-24 (proven against the order's gerbers); `eagle/deprecated/v1.3-do-not-use` is an earlier save without IC15 (the 74245 enable) and with the TMP registers off the board |
...
7. OPEN 2026-09-24: the tree's microcode gained `LDZ Rn,d` / `STZ Rn,d` ($80-$8F, the word at R6.hi:d), `ADDIW Rn,#w`
   ...
   records are the old all-zero words on the machine (review H-4). Reload with `tools/ucode_send.py --all` (Ken, at the
