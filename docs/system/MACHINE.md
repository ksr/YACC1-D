...
|---|---|---|
| Memory card 28C64 EEPROM | the 2021 build (git ff7d85a 2021-07-10) = `firmware/rom/eprom-captured-2026-09-18.bin/.hex`; **`firmware/rom/shipped/rom` is now the 2026-09-22 rebuild TO BURN** (monitor G = JSRUR R7 instead of BRVR R7; the T-menu tests removed; the CompactFlash driver, the `O` boot command and vectors CFINIT/CFREAD/CFWRITE/CONST at $FFEC..$FFF8 — `firmware/abi/README.md`; BASIC unchanged) | chip captured 2026-09-18 = the 2021 sources; the rebuild reproduces from `firmware/monitor/monitor.asm` (`tools/verify_firmware.py`). Until the chip is reburned, `tests/memory/rom_verify.py` / `memory_status.py` will report the monitor half differs (they compare the chip with the tree's images) |
| Sequencer microcode (RAM/EEPROM on the sequencer-memory card) | the 2026-09-21 image = `firmware/microcode/ucode-generator2/test.hex` as committed BEFORE 2026-09-22; **the tree's `test.hex` now differs in 14 records — BRUR at $AD, the H-2 fix (BRZ/BRNZ/BR16Z/BR16NZ release -AC-RD before the PC load) and the H-1 fix (PUSHR releases the register card before its stack writes) — and must be reloaded**; both fixes were reproduced and verified on the microcode-level emulator `software/ucemu` (Sequencer4 download mode / the Processing loader with `test.hexz`) — **EEPROM verified 2026-09-21** against the previous image: the boot dumps of instructions 0, 1, 7, 124, 255 equal test.hex byte for byte, and the RAM copy equals them (`tests/sequencer/`) | the loader's `cache` = what was last sent = this image; regenerated byte-identical from the generator |
