...
disagree about an instruction.
  ...
  gen_y1_optab.py [DEF] [-o os/asm_optab.c]      write the tables (default: yacc1.def -> os/asm_optab.c and
                                                   os/commands-asm/asmtab.inc)
  gen_y1_optab.py --check                          exit 1 if either is not what yacc1.def gives (tests/asm)

Two outputs from one reading of the .def (2026-09-26): os/asm_optab.c for the C assembler os/commands/asm.c (/BIN/ASMC)
and os/commands-asm/asmtab.inc, the same patterns and operations as numeric DB lines for the assembly-language
assembler os/commands-asm/asm.asm (/BIN/ASM), which INCLUDEs it (format: render_asm()).
...
OUT_ASM = os.path.join(ROOT, "os/commands-asm/asmtab.inc")
...
def rotl_hash(name, seed):
    """asm.asm's hash (2026-09-26): h = seed, then h = rotl8(h) + c over the bytes (8 bits); the chain is h & 63. The
    same function, seed 0 and all 8 bits, is its label hash (256 chains)."""
    h = seed
    for c in name.encode("latin1"):
        h = (((h << 1) | (h >> 7)) + c) & 255
    return h
...
def render_asm(path):
    """os/commands-asm/asmtab.inc: the table of build() for asm.asm, as DB/DW lines (numbers only: the assembler
    upper-cases its source; every line well under RC/asm's 100 characters).
      OT_SEED        EQU: the mnemonic hash's seed (rotl_hash), the first that keeps every chain at 3 records or fewer
                     with the fewest records passed on the way to all of them
      OT_HEAD        64 words: the first record of each chain, 0 = none
      OT_CLS         the operand classes' tables (DW), shape byte 8+k uses OT_CLS[k]; a class table is 0 and
                     entries `len name value`, then 0; or, when every name is 2 characters with the same first one,
                     1, that character, the lowest second character, n, then n values by second character (255 none)
      OT_Mn          a mnemonic record: DB len, DW the next record of its chain (0 = the last), DB the name, 127, the
                     number of patterns, then per pattern its operand shape (ends with 0), the number of operation
                     bytes and the operations, all as in asm_optab.c."""
    title, classes, trans, order, groups, cls, buckets, seed = build(path)
    best = None
    for sd in range(256):
        ch = [[] for _ in range(OPHASH)]
        for name in order: ch[rotl_hash(name, sd) & 63].append(name)
        if max(len(c) for c in ch) > 3: continue
        cost = sum(len(c) * (len(c) + 1) // 2 for c in ch)
        if best is None or cost < best[0]: best = (cost, sd, ch)
    if best is None: sys.exit("gen_y1_optab: no seed keeps asm.asm's mnemonic chains at 3 or fewer")
    _, sd, chains = best
    ids = {name: i for i, name in enumerate(order)}
    rel = os.path.relpath(path, ROOT)
    out = ["; asmtab.inc - GENERATED by tools/gen_y1_optab.py from %s: do not edit, run the generator" % rel,
           "; (os/Makefile does, and tests/asm/run.py checks that this file is current). The YACC1 instruction table",
           "; of /BIN/ASM (os/commands-asm/asm.asm): %d patterns of RC/asm's .def file (%s) under %d mnemonics," % (
               len(trans), title.strip(), len(order)),
           "; %d operand classes; the same patterns and operations as os/asm_optab.c (the formats are in the" % len(classes),
           "; generator: render_asm)."
           , "OT_SEED:    EQU %d" % sd]
    heads = []
    for c in chains: heads.append("OT_M%d" % ids[c[0]] if c else "0")
    out.append("OT_HEAD:")
    for i in range(0, 64, 8): out.append("        DW " + ",".join(heads[i:i + 8]))
    out.append("OT_CLS:     DW " + ",".join("OT_C%d" % k for k in range(len(cls))))

    def dbs(label, vals, comment=""):
        lines = []
        for i in range(0, len(vals), 16):
            lines.append("%-12sDB %s" % (label + ":" if i == 0 and label else "", ",".join(str(v) for v in vals[i:i + 16])))
        if not vals: lines.append(label + ":")
        if comment: lines[0] = "%-72s; %s" % (lines[0], comment)
        return lines
    for k, (name, entries) in enumerate(classes):
        names = [t for t, _ in entries]
        if (all(len(t) == 2 and t[0] == names[0][0] for t in names) and len(set(names)) == len(names)
                and all(v != 255 for _, v in entries)):
            # direct: every name is 2 characters with the same first one, so at most one can match (Class_Match's
            # "first entry the text starts with" is that one): 1, the first character, the lowest second character,
            # the number of values, then the value of each second character from the lowest (255 = none)
            lo = min(ord(t[1]) for t in names); hi = max(ord(t[1]) for t in names)
            tab = [255] * (hi - lo + 1)
            for t, v in entries: tab[ord(t[1]) - lo] = v
            out += dbs("OT_C%d" % k, [1, ord(names[0][0]), lo, len(tab)] + tab, "class " + name + " (direct)")
        else:                                        # linear: 0, then `len name value` in .def order, then 0
            vals = [0]
            for t, v in entries: vals += [len(t)] + [ord(ch) for ch in t] + [v]
            out += dbs("OT_C%d" % k, vals + [0], "class " + name)
    nxt = {}
    for c in chains:
        for a, b in zip(c, c[1:]): nxt[a] = b
    for name in order:
        i = ids[name]
        out += dbs("OT_M%d" % i, [len(name)], name)
        out.append("            DW %s" % ("OT_M%d" % ids[nxt[name]] if name in nxt else "0"))
        if "\x7f" in name: sys.exit("gen_y1_optab: a DEL in mnemonic %r (asm.asm's name terminator)" % name)
        rec = [ord(ch) for ch in name] + [127, len(groups[name])]   # 127: the name's end (never a blank, never in one)
        for sh, ops, _ in groups[name]: rec += sh + [0, len(ops)] + ops
        out += dbs("", rec)
    return "\n".join(out) + "\n"
...
def main():
    ...
    outs = [(out, render(path))]
    if out == OUT: outs.append((OUT_ASM, render_asm(path)))       # (-o writes the C table alone)
    bad = 0
    for o, s in outs:
        old = open(o).read() if os.path.exists(o) else None
        if check:
            if old != s:
                print("gen_y1_optab: %s is not current with %s (run tools/gen_y1_optab.py)" % (os.path.relpath(o, ROOT), os.path.relpath(path, ROOT)))
                bad = 1
            else: print("gen_y1_optab: %s is current" % os.path.relpath(o, ROOT))
            continue
        if old != s: open(o, "w").write(s)
        print("gen_y1_optab: %s -> %s (%s)" % (os.path.relpath(path, ROOT), os.path.relpath(o, ROOT), "written" if old != s else "unchanged"))
    if bad: sys.exit(1)
