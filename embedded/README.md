...
| `libraries/` | – | VENDORED: `Adafruit_MCP23017_Arduino_Library` 1.1.0 (the 1.x API every sketch here uses) and `YACC/YACC_Common_header.h` (the bus signal table the tester sketches include; `-pre3-2.h` = its previous version). Install both in `~/Documents/Arduino/libraries/` | – |

Build check (arduino-cli, Uno): `arduino-cli compile --fqbn arduino:avr:uno embedded/bus-tester/bus-driver` after copying
`libraries/*` into the Arduino libraries folder. The sequencer sketches carry their own copy of the signal subset
