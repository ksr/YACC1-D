...
#define VERSION_BANNER "Sequencer4 2026-09-21 (400 kHz I2C, block EEPROM reads, verified copy)"
#define VERIFY_COPY   // read the RAM back and compare with the EEPROM before READY
...
void setup() {
  ...
  if (mode == WRITEMEM) {
    Serial.println(F("\n\n\n\nStartup"));
    Serial.println(F(VERSION_BANNER));
  }

  for (i = 0; i < NUMBER_OF_CONTROLERS; i++) {
    mcp[i].begin(i);
  }
  Wire.setClock(400000);   // AFTER the expanders' begin(): each Adafruit begin() calls Wire.begin(), which resets the clock to 100 kHz. EEPROM (24LC) and MCP23017s are 400 kHz parts
...
//#define FILLRAM   // Sequencer4: the test-pattern fill of every instruction before the copy is off (it only cost time)
#ifdef FILLRAM
    ...
    Serial.println(F("EEPROM to RAM"));
    digitalWrite(LOADING, HIGH);
    setAddressOutput();
    setDataOutput();
    unsigned long t0 = millis();
    for (instruction = 0; instruction < WORKING_INSTRUCTION_SET; instruction++) {
      if ((instruction & 31) == 0) { Serial.print(F("Copying Instruction ")); Serial.println(instruction); }
      digitalWrite(LOADING, statusLed);
      statusLed = !statusLed;
      readCodeFromROM(instruction, insData);
      writeInstruction(instruction, insData);
    }
    Serial.print(F("- done, ")); Serial.print((millis() - t0) / 1000); Serial.println(F(" s"));

#ifdef VERIFY_COPY
    Serial.println(F("Verify RAM against EEPROM"));
    t0 = millis();
    setDataInput();
    setAddressOutput();
    uCodeRamRead(true);
    uCodeRamSelect(true);
    {
      unsigned char romChunk[32];   // the EEPROM side is compared 32 bytes at a time: only ~500 bytes of stack are free
      unsigned int badInstructions = 0, badBytes = 0, firstBad = 0xFFFF;
      for (instruction = 0; instruction < WORKING_INSTRUCTION_SET; instruction++) {
        digitalWrite(LOADING, statusLed);
        statusLed = !statusLed;
        readInstruction(instruction, insData);           // RAM -> insData (global, 512 bytes)
        unsigned int bad = 0;
        for (unsigned int off = 0; off < INSTRUCTION_SIZE; off += sizeof romChunk) {
          readCodeFromROMPart(instruction, off, sizeof romChunk, romChunk);
          for (i = 0; i < (int)sizeof romChunk; i++) if (insData[off + i] != romChunk[i]) bad++;
        }
        if (bad) { badInstructions++; badBytes += bad; if (firstBad == 0xFFFF) firstBad = instruction; }
      }
      Serial.print(F("- ")); Serial.print((millis() - t0) / 1000); Serial.print(F(" s, "));
      if (badInstructions == 0) Serial.println(F("RAM == EEPROM for all 256 instructions"));
      else {
        Serial.print(F("MISMATCH: ")); Serial.print(badInstructions); Serial.print(F(" instructions, ")); Serial.print(badBytes);
        Serial.print(F(" bytes differ, first at instruction ")); Serial.println(firstBad);
        Serial.println(F("RAM copy FAILED - is -BUS-EN inactive? (the logic card drives the microcode address lines when it is asserted)"));
        uCodeRamSelect(false);
        setAddressInput();
        doError("RAM verify failed", 6);   // never returns: FAULT blinks 6, READY stays low
      }
    }
#endif
...
#define DISPLAY_MEM
    ...
    Serial.println(F("RAM copy complete and verified, READY!!!"));
