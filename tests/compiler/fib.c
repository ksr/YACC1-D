...
void main() {
    ...
    for (i = 0; i < 25; i++) { putnum(fibs[i]); putchar(i == 24 ? '\n' : ' '); }   /* wraps after 46368 */
    putnum(gcd(1071, 462)); putchar(' '); putnum(gcd(65535, 255)); putchar(' '); putnum(gcd(17, 5)); putchar('\n');   /* 21 255 1 */
    for (i = 1; i <= 12; i++) { putnum(i * i); putchar(':'); putnum(digitsum(i * i)); putchar(' '); }
    putchar('\n');
    for (i = 0; i < 6; i++) { puthex(fibs[i * 4]); putchar(' '); }
    putchar('\n');
}
