...
"BUS-OFF",SPECIAL_OPCODE,0,0,
"BUS-ON",SPECIAL_OPCODE,0,0,
"",0,0,0,
};
...
void loop() {
  ...
  if(commandReady){
    ...
    if(opcodes[opcodeIndex].chip != SPECIAL_OPCODE){
      ...
      writeAddress(val);
    else if (strcmp(opcode,"BUS-OFF")==0)
      busctl(0);
    else if (strcmp(opcode,"BUS-ON")==0)
      busctl(1);
    else
...
void busctl(int state){
  int val;
  int i;
  Serial.print("BUS-CTL: ");
