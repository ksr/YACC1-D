...
void endInstruction() {
    ...
    eolInfo[currentInstruction] = ucodeLine;
}
...
int main(int argc, char** argv) {
    ...
    // out on
    setBit("OUT-ON");
    //setBit("HALT"); added for testing HALT function
    writeCurrentLine();
    ...
    setBit("ADDR-REG-FUNC-RD"); //sp/pc sel on old card try without ??? should be off
    setBit("ADDR-REG-ADDR-RD");
    setBit("MEM-RD");
    writeCurrentLine();
    setBit("BRANCH-LD-HI");
